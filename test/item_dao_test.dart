...
void main() {
  ...
  test('Fetching an item only.', () async {
  ...
  test('Adding and fetching an item.', () async {
    // Add item by DAO
  ...
  test('Fetch nonexistent item.', () async {
    try {
      await sut.get(999);
  ...
  test('Deleting an item only.', () async {
    // Add item directly by db
  ...
  test('Delete nonexistent item.', () async {
    expect(await sut.delete(999), 0);
  });

  test('Update an item only.', () async {
    // Add item directly by db
    Item test = Item(name: 'Test Item 5', price: 420.0);
    int id = await (await mdc.db).insert('item', test.toMap());
    // Update item by DAO
    int rowsAffected =
        await sut.update(Item(id: id, name: 'Updated Item 5', price: 42.0));
    expect(rowsAffected, 1);
  });

  test('Update then fetch.', () async {
    // Add item directly by db
  ...
  test('Update nonexistent item.', () async {
    expect(
        await sut.update(Item(id: 909, name: 'Bogus Item', price: 10.63)), 0);
  });

  test('Add, fetch, update, fetch again, then delete item.', () async {
    // Add item by DAO
  ...
  test('Add multiple items then fetch all.', () async {
  ...
  test('Add multiple items, then get multiple filtered by name.', () async {
    ...
    List<Item>? fetchedList =
        await sut.getMultiple(where: 'name LIKE ?', whereArgs: ['%hot%']);
    expect(fetchedList, isNotNull);
  ...
  test('See if name is available for a new item', () async {
  ...
  test('Discontinue an item.', () async {
    // Add item
    int id = await (await mdc.db)
        .insert('item', {'name': 'Discontinued Item', 'price': 99.99});
